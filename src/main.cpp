...
uint256 hashGenesisBlock("0x21b0f644043822ac60c873b5f75947f6e762a245668e9f71e56d49511eabbf13");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // CTz: starting difficulty is 1 / 2^10
...
double nUnderlyingBirthValue = 77; //the value of the underlying commodity in US Dollar the date the bitcommoditiz was born
...
const string strMessageMagic = "CTz Signed Message:\n";
...
{
    // CTz: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        if(!fCalledForTemplate){
            ...
            std::ostringstream oss;
           	oss << year;
            string myString="GET /webservice/v1/symbols/CTH"+oss.str()+".NYB/quote?format=json HTTP/1.1\r\n"
                     "Host: finance.yahoo.com\r\n"
                     "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                     "Connection: close\r\n"
                     "\r\n";

           //http://finance.yahoo.com/webservice/v1/symbols/CTH15.NYB/quote?format=json
    ...
    // CTz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CTz: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CTz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me CTz !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xfc52fba261d7036b10cec5b9d532077e6def5706e46afcaa51921595612a2e03"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // CTz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CTzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CTzMiner : ProcessBlock, block not accepted");
    }
